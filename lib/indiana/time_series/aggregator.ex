defmodule Indiana.TimeSeries.Aggregator do
  @name :indiana_time_series_aggregator
  @default_interval 60
  @env Mix.env

  @moduledoc """
  `Indiana.TimeSeries.Aggregator` is the aggregation framework for time series. It's purposed mostly for internal use only.
  ...
  ## Examples
    iex> Indiana.TimeSeries.Aggregator.last_aggregated_at
    1442946540000000
  ...
  """
  def last_aggregated_at, do: GenServer.call(@name, :last_aggregated_at)
  ...
  def aggregate(data, before_time: before_time, after_time: after_time) when is_list(data) do
    ...
    |> batch_by(interval: @default_interval)
    |> calculate_dimensions(interval: @default_interval)
  end
  ...
  def aggregate(time_series, before_time: before_time, after_time: after_time) do
    Indiana.TimeSeries.get(time_series)
    |> Indiana.TimeSeries.Aggregator.aggregate(before_time: before_time, after_time: after_time)
    |> Enum.map(fn({time, value}) -> Indiana.TimeSeries.Aggregated.insert(time_series, {time, value}) end)
  end

  @doc false
  defp filter_by(data, before_time: before_time, after_time: after_time) do
    Enum.filter(data, fn({time, _value}) -> time >= after_time && time < before_time end)
  end

  @doc false
  defp batch_by(data, interval: interval) do
    Enum.group_by(data, fn({time, _value}) -> div(time, interval * 1000000) end)
  end
  ...
  def init(:ok) do
    ...
    {:ok, Indiana.Time.last_full_minute}
  end

  @doc false
  def handle_call(:last_aggregated_at, _from, last_aggregated_at), do: {:reply, last_aggregated_at, last_aggregated_at}

  @doc false
  def handle_info(:schedule_aggregation, last_aggregated_at) do
    Indiana.TimeSeries.all
    |> Map.keys
    |> Enum.each(fn(key) -> aggregate(key, before_time: Indiana.Time.last_full_minute, after_time: last_aggregated_at) end)

    {:noreply, Indiana.Time.last_full_minute}
  end
end
